(ns metabase.driver.sample-driver
  "Sample Metabase 3rd party driver.  Functionally identical to the included :postgres driver (as of Feb 2022)."
...
(driver/register! :sample-driver, :parent :sql-jdbc)
...
(defmethod sql.qp/add-interval-honeysql-form :sample-driver
  [_ hsql-form amount unit]
  (hx/+ (->timestamp hsql-form)
        (hsql/raw (format "(INTERVAL '%s %s')" amount (name unit)))))

(defmethod driver/humanize-connection-error-message :sample-driver
  [_ message]
...
(defmethod driver.common/current-db-time-date-formatters :sample-driver
  [_]
  (driver.common/create-db-time-formatters "yyyy-MM-dd HH:mm:ss.SSS zzz"))

(defmethod driver.common/current-db-time-native-query :sample-driver
  [_]
  "select to_char(current_timestamp, 'YYYY-MM-DD HH24:MI:SS.MS TZ')")

(defmethod driver/current-db-time :sample-driver
  [& args]
  (apply driver.common/current-db-time args))

(defmethod driver/connection-properties :sample-driver
  [_]
  (->>
...
(defmethod driver/db-start-of-week :sample-driver
  [_]
  :monday)
...
(defmethod driver/describe-table :sample-driver
...
(defmethod sql.qp/unix-timestamp->honeysql [:sample-driver :seconds]
  [_ _ expr]
  (hsql/call :to_timestamp expr))

(defmethod sql.qp/cast-temporal-string [:sample-driver :Coercion/YYYYMMDDHHMMSSString->Temporal]
  [_driver _coercion-strategy expr]
  (hsql/call :to_timestamp expr (hx/literal "YYYYMMDDHH24MISS")))

(defmethod sql.qp/cast-temporal-byte [:sample-driver :Coercion/YYYYMMDDHHMMSSBytes->Temporal]
...
(defmethod sql.qp/date [:sample-driver :default]         [_ _ expr] expr)
(defmethod sql.qp/date [:sample-driver :minute]          [_ _ expr] (date-trunc :minute expr))
(defmethod sql.qp/date [:sample-driver :minute-of-hour]  [_ _ expr] (extract-integer :minute expr))
(defmethod sql.qp/date [:sample-driver :hour]            [_ _ expr] (date-trunc :hour expr))
(defmethod sql.qp/date [:sample-driver :hour-of-day]     [_ _ expr] (extract-integer :hour expr))
(defmethod sql.qp/date [:sample-driver :day]             [_ _ expr] (hx/->date expr))
(defmethod sql.qp/date [:sample-driver :day-of-month]    [_ _ expr] (extract-integer :day expr))
(defmethod sql.qp/date [:sample-driver :day-of-year]     [_ _ expr] (extract-integer :doy expr))
(defmethod sql.qp/date [:sample-driver :month]           [_ _ expr] (date-trunc :month expr))
(defmethod sql.qp/date [:sample-driver :month-of-year]   [_ _ expr] (extract-integer :month expr))
(defmethod sql.qp/date [:sample-driver :quarter]         [_ _ expr] (date-trunc :quarter expr))
(defmethod sql.qp/date [:sample-driver :quarter-of-year] [_ _ expr] (extract-integer :quarter expr))
(defmethod sql.qp/date [:sample-driver :year]            [_ _ expr] (date-trunc :year expr))

(defmethod sql.qp/date [:sample-driver :day-of-week]
  [_ _ expr]
  (sql.qp/adjust-day-of-week :sample-driver (extract-integer :dow expr)))

(defmethod sql.qp/date [:sample-driver :week]
  [_ _ expr]
  (sql.qp/adjust-start-of-week :sample-driver (partial date-trunc :week) expr))

(defmethod sql.qp/->honeysql [:sample-driver :value]
  [driver value]
...
(defmethod sql.qp/->honeysql [:sample-driver :median]
  [driver [_ arg]]
...
(defmethod sql.qp/->honeysql [:sample-driver :regex-match-first]
  [driver [_ arg pattern]]
  (let [identifier (sql.qp/->honeysql driver arg)]
    (->RegexMatchFirst identifier pattern)))

(defmethod sql.qp/->honeysql [:sample-driver Time]
  [_ time-value]
  (hx/->time time-value))
...
(defmethod sql.qp/->honeysql [:sample-driver :field]
...
(defmethod unprepare/unprepare-value [:sample-driver Date]
  [_ value]
  (format "'%s'::timestamp" (u.date/format value)))

(prefer-method unprepare/unprepare-value [:sql Time] [:sample-driver Date])

(defmethod unprepare/unprepare-value [:sample-driver UUID]
  [_ value]
  (format "'%s'::uuid" value))
...
(defmethod sql-jdbc.sync/database-type->base-type :sample-driver
  [driver column]
  (if (contains? *enum-types* column)
    :type/PostgresEnum
    (default-base-types column)))

(defmethod sql-jdbc.sync/column->semantic-type :sample-driver
  [_ database-type _]
...
(defn- ssl-params
  ...
  (let [ssl-root-cert   (when (contains? #{"verify-ca" "verify-full"} (:ssl-mode db-details))
    ...
    (cond-> (set/rename-keys db-details {:ssl-mode :sslmode})
      ...
      (assoc :sslrootcert (secret/value->file! ssl-root-cert :sample-driver))

      (has-value? ssl-client-key)
      (assoc :sslkey (secret/value->file! ssl-client-key :sample-driver))

      (has-value? ssl-client-cert)
      (assoc :sslcert (secret/value->file! ssl-client-cert :sample-driver))
...
(defmethod db.spec/spec :sample-driver
  [_ {:keys [host port db]
  ...
  (merge
   {:classname                     "org.postgresql.Driver"
    ...
    :subname                       (db.spec/make-subname host (or port 5432) db)
...
(defmethod sql-jdbc.conn/connection-details->spec :sample-driver
  ...
  (let [props (-> details-map
        ...
        props (as-> props it
                ...
                (db.spec/spec :sample-driver it)
                (sql-jdbc.common/handle-additional-options it details-map))]
    props))

(defmethod sql-jdbc.execute/set-timezone-sql :sample-driver
  [_]
...
(defmethod sql-jdbc.execute/read-column-thunk [:sample-driver Types/TIMESTAMP]
...
(defmethod sql-jdbc.execute/read-column-thunk [:sample-driver Types/TIME]
...
(defmethod sql-jdbc.execute/read-column-thunk [:sample-driver Types/DOUBLE]
...
(defmethod sql-jdbc.execute/read-column-thunk :sample-driver
...
(defmethod sql-jdbc.execute/set-parameter [:sample-driver OffsetTime]
