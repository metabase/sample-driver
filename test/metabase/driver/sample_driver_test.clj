(ns metabase.driver.sample-driver-test
  "Tests for features/capabilities specific to the sample driver."
...
(defn- drop-if-exists-and-create-db!
  ...
  [db-name]
  (let [spec (sql-jdbc.conn/connection-details->spec :sample-driver (mt/dbdef->connection-details :sample-driver :server nil))]
...
(deftest connection-details->spec-test
  (testing (str "Check that SSL params get added the connection details in the way we'd like # no SSL -- this should "
    ...
    (is (= {:classname                     "org.postgresql.Driver"
           ...
           (sql-jdbc.conn/connection-details->spec :sample-driver
             {:ssl    false
              :host   "localhost"
              :port   5432
...
(deftest edge-case-identifiers-test
  (mt/test-driver :sample-driver
    ...
    (testing "make sure schema/table/field names with hyphens in them work correctly (#8766)"
      (let [details (mt/dbdef->connection-details :sample-driver :db {:database-name "hyphen-names-test"})
            spec    (sql-jdbc.conn/connection-details->spec :sample-driver details)]
        ;; create the postgres DB
        (drop-if-exists-and-create-db! "hyphen-names-test")
        ;; create the DB object
        (mt/with-temp Database [database {:engine :sample-driver, :details (assoc details :dbname "hyphen-names-test")}]
...
(deftest materialized-views-test
  (mt/test-driver :sample-driver
    (testing (str "Check that we properly fetch materialized views. As discussed in #2355 they don't come back from "
      ...
      (let [details (mt/dbdef->connection-details :sample-driver :db {:database-name "materialized_views_test"})]
        (jdbc/execute! (sql-jdbc.conn/connection-details->spec :sample-driver details)
                       ["DROP MATERIALIZED VIEW IF EXISTS test_mview;
                       CREATE MATERIALIZED VIEW test_mview AS
                       SELECT 'Toucans are the coolest type of bird.' AS true_facts;"])
        (mt/with-temp Database [database {:engine :sample-driver, :details (assoc details :dbname "materialized_views_test")}]
          (is (= {:tables #{(default-table-result "test_mview")}}
                 (driver/describe-database :sample-driver database))))))))

(deftest foreign-tables-test
  (mt/test-driver :sample-driver
    (testing "Check that we properly fetch foreign tables."
      (drop-if-exists-and-create-db! "fdw_test")
      (let [details (mt/dbdef->connection-details :sample-driver :db {:database-name "fdw_test"})]
        (jdbc/execute! (sql-jdbc.conn/connection-details->spec :sample-driver details)
        ...
        (mt/with-temp Database [database {:engine :sample-driver, :details (assoc details :dbname "fdw_test")}]
...
(deftest recreated-views-test
  (mt/test-driver :sample-driver
    (testing (str "make sure that if a view is dropped and recreated that the original Table object is marked active "
      ...
      (let [details (mt/dbdef->connection-details :sample-driver :db {:database-name "dropped_views_test"})
            spec    (sql-jdbc.conn/connection-details->spec :sample-driver details)]
        ;; create the postgres DB
        (drop-if-exists-and-create-db! "dropped_views_test")
        ;; create the DB object
        (mt/with-temp Database [database {:engine :sample-driver, :details (assoc details :dbname "dropped_views_test")}]
          (let [sync! #(sync/sync-database! database)]
            ;; populate the DB and create a view
  ...
  (mt/test-driver :sample-driver
    (testing (str "Make sure that partitioned tables (in addition to the individual partitions themselves) are
      ...
      (let [db-name "partitioned_table_test"
            details (mt/dbdef->connection-details :sample-driver :db {:database-name db-name})
            spec    (sql-jdbc.conn/connection-details->spec :sample-driver details)]
        ;; create the postgres DB
        ...
        (let [major-v ((jdbc/with-db-metadata [metadata spec]
          ...
          (if (>= major-v 10)
            ;; create the DB object
            (mt/with-temp Database [database {:engine :sample-driver, :details (assoc details :dbname db-name)}]
              (let [sync! #(sync/sync-database! database)]
...
(defn- do-with-money-test-db [thunk]
  ...
  (let [details (mt/dbdef->connection-details :sample-driver :db {:database-name "money_columns_test"})]
    (jdbc/with-db-connection [conn (sql-jdbc.conn/connection-details->spec :sample-driver details)]
    ...
    (mt/with-temp Database [db {:engine :sample-driver, :details (assoc details :dbname "money_columns_test")}]
  ...
  (mt/test-driver :sample-driver
    (testing "We should support the Postgres MONEY type"
      (testing "It should be possible to return money column results (#3754)"
        (with-open [conn (sql-jdbc.execute/connection-with-timezone :sample-driver (mt/db) nil)
                    stmt (sql-jdbc.execute/prepared-statement :sample-driver conn "SELECT 1000::money AS \"money\";" nil)
                    rs   (sql-jdbc.execute/execute-prepared-statement! :sample-driver stmt)]
          (let [row-thunk (sql-jdbc.execute/row-thunk :sample-driver rs (.getMetaData rs))]
            (is (= [1000.00M]
                   (row-thunk))))))
...
(defn- enums-test-db-details [] (mt/dbdef->connection-details :sample-driver :db {:database-name "enums_test"}))
...
(defn- create-enums-db!
  ...
  (jdbc/with-db-connection [conn (sql-jdbc.conn/connection-details->spec :sample-driver (enums-test-db-details))]
...
(defn- do-with-enums-db {:style/indent 0} [f]
  (create-enums-db!)
  (mt/with-temp Database [database {:engine :sample-driver, :details (enums-test-db-details)}]
  ...
  (mt/test-driver :sample-driver
    (testing "check that values for enum types get wrapped in appropriate CAST() fn calls in `->honeysql`"
      (is (= (hx/with-database-type-info (hsql/call :cast "toucan" (keyword "bird type")) "bird type")
             (sql.qp/->honeysql :sample-driver [:value "toucan" {:database_type "bird type", :base_type :type/PostgresEnum}]))))

    (do-with-enums-db
      (fn [db]
        (testing "check that we can actually fetch the enum types from a DB"
          (is (= #{(keyword "bird type") :bird_status}
                 (#'postgres/enum-types :sample-driver db))))
          ...
          (is (= {:name   "birds"
                  :fields #{{:name              "name"
                            ...
                            {:name              "type"
                             ...
                             :base-type         :type/PostgresEnum
                             :database-position 2}}}
                 (driver/describe-table :sample-driver db {:name "birds"}))))
...
(deftest fingerprint-time-fields-test
  (mt/test-driver :sample-driver
    (testing "Make sure we're able to fingerprint TIME fields (#5911)"
      ...
      (let [details (mt/dbdef->connection-details :sample-driver :db {:database-name "time_field_test"})]
        (jdbc/execute! (sql-jdbc.conn/connection-details->spec :sample-driver details)
        ...
        (mt/with-temp Database [database {:engine :sample-driver, :details (assoc details :dbname "time_field_test")}]
...
(deftest dont-sync-tables-with-no-select-permissions-test
  (testing "Make sure we only sync databases for which the current user has SELECT permissions"
    (mt/test-driver :sample-driver
      ...
      (let [details (mt/dbdef->connection-details :sample-driver :db {:database-name "no-select-test"})
            spec    (sql-jdbc.conn/connection-details->spec :sample-driver details)]
        (doseq [statement ["CREATE TABLE PUBLIC.table_with_perms (x INTEGER NOT NULL);"
          ...
          (jdbc/execute! spec [statement])))
      (let [test-user-details (assoc (mt/dbdef->connection-details :sample-driver :db {:database-name "no-select-test"})
                                     :user "no_select_test_user"
                                     :password "123456")]
        (mt/with-temp Database [database {:engine :sample-driver, :details test-user-details}]
          (sync/sync-database! database)
          (is (= #{"table_with_perms"}
...
(deftest json-operator-?-works
  (testing "Make sure the Postgres ? operators (for JSON types) work in native queries"
    (mt/test-driver :sample-driver
      ...
      (let [details (mt/dbdef->connection-details :sample-driver :db {:database-name "json-test"})
            spec    (sql-jdbc.conn/connection-details->spec :sample-driver details)]
        (doseq [statement ["DROP TABLE IF EXISTS PUBLIC.json_table;"
          ...
          (jdbc/execute! spec [statement])))
      (let [json-db-details (mt/dbdef->connection-details :sample-driver :db {:database-name "json-test"})
            query           (str "SELECT json_val::jsonb ? 'a',"
                                 "json_val::jsonb ?| array['c', 'd'],"
                                 "json_val::jsonb ?& array['a', 'b']"
                                 "FROM \"json_table\";")]
        (mt/with-temp Database [database {:engine :sample-driver, :details json-db-details}]
